...
mod tests {
    ...
    // Default test timers
    const TEST_TURN_TIMER: u32 = 30;
    const TEST_POISON_TIMER: u32 = 60;
    ...
    async fn test_create_game() {
        ...
        assert_eq!(game.turn_timer_secs, TEST_TURN_TIMER);
        assert_eq!(game.poison_timer_secs, TEST_POISON_TIMER);
    }

    #[tokio::test]
    async fn test_create_game_with_custom_timer() {
        let engine = GameEngine::new();
        let game_id = engine
            .create_game(
                "P1".to_string(),
        ...
        let game = engine.get_game(game_id).unwrap();
        assert_eq!(game.turn_timer_secs, 10);
    }
    ...
    async fn test_set_poison() {
        ...
        let result = engine
            .set_poison_choice(game_id, p1_id, &p1_candy)
            .await
            .unwrap();
        ...
        let result = engine
            .set_poison_choice(game_id, p2_id, &p2_candy)
            .await
            .unwrap();
    ...
    async fn test_win_by_threshold() {
        ...
        // Set poisons
        let _ = engine
            .set_poison_choice(game_id, p1_id, &p1_poison)
            .await
            .unwrap();
        let _ = engine
            .set_poison_choice(game_id, p2_id, &p2_poison)
            .await
            .unwrap();

        let game = engine.get_game(game_id).unwrap();
        let p2_candies: Vec<String> = game
            .player2
            .owned_candies
            .iter()
            ...
            .cloned()
            .collect();
        let p1_candies: Vec<String> = game
            .player1
            .owned_candies
            .iter()
        ...
        for i in 0..10 {
            let _ = engine
                .make_move(game_id, p1_id, &p2_candies[i])
                .await
                .unwrap();
            let _ = engine
                .make_move(game_id, p2_id, &p1_candies[i])
                .await
                .unwrap();
        }
        ...
        // P1 picks 11th candy
        let result = engine
            .make_move(game_id, p1_id, &p2_candies[10])
            .await
            .unwrap();
        ...
        let result = engine
            .make_move(game_id, p2_id, &p1_candies[10])
            .await
            .unwrap();
        assert!(result.game_over);
