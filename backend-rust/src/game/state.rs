...
impl GameSession {
    ...
    pub fn new(
        ...
        let p1_candies: std::collections::HashSet<String> =
            pool[0..12].iter().map(|&s| s.to_string()).collect();
    ...
    pub fn both_poisons_set(&self) -> bool {
        matches!(
            (&self.player1.poison_choice, &self.player2.poison_choice),
            (Some(_), Some(_))
        )
    }
    ...
    pub fn for_viewer(&self, viewer_id: Uuid) -> serde_json::Value {
        ...
        // Sanitize players - construct manually by modifying the Map
