...
async def guest_login(request: GuestAuthRequest = None, db_service=Depends(get_db_service)):
    ...
    - See or claim quests
    - Add friends
    - Have persistent stats (unless they later register)
    """
    try:
        ...
        # Create guest user object (not persisted to DB by default)
        guest_user = {
            "id": user_id,
            "name": guest_name,
            "email": None,
        ...
        # Generate tokens (shorter expiry for guests - 24 hours)
        from datetime import timedelta
        access_token = create_access_token(
            data={"sub": user_id, "is_guest": True},
            expires_delta=timedelta(hours=24)
        )
        # No refresh token for guests - they can just get a new guest session
        
        logger.info(f"✅ Guest session created: {guest_name}")
