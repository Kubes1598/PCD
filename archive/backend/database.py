...
class InMemoryDatabaseService(BaseDatabaseService):
    ...
    async def get_guest_by_device_id(self, device_id: str) -> Optional[Dict[str, Any]]:
...
class SupabaseService(BaseDatabaseService):
    ...
    async def get_leaderboard(self, sort_by: str = "wins", limit: int = 10) -> List[Dict[str, Any]]:
        ...
        # Try to use materialized view for better performance (schema v3+)
        try:
            if sort_by == "wins":
                # Use optimized materialized view with pre-computed stats
                res = self.supabase.from_("player_stats_mv").select("*").limit(limit).execute()
            else:
                # Fallback to direct table query for coin balance sorting
                res = self.supabase.table("players").select("*").order("coin_balance", desc=True).limit(limit).execute()
        except Exception as e:
            # Fallback if materialized view doesn't exist (pre-v3 schema)
            print(f"⚠️ Materialized view not available, using direct query: {e}")
    ...
    async def refresh_analytics(self) -> bool:
        """
        Refresh materialized views for analytics (schema v3+).
        ...
        """
        try:
            self.supabase_admin.rpc("refresh_analytics_views").execute()
            print("✅ Analytics views refreshed successfully")
            
            # Clear leaderboard cache after refresh
            try:
                r = await redis_client.connect()
                await r.delete("pcd:leaderboard_cache:wins:*")
            except:
                pass
            
            return True
        except Exception as e:
            print(f"⚠️ Failed to refresh analytics views (may not exist in pre-v3 schema): {e}")
            return False
    ...
    async def get_guest_by_device_id(self, device_id: str) -> Optional[Dict[str, Any]]:
        ...
        Enables persistent guest sessions across app restarts.
        """
        # Check Cache
        try:
            r = await redis_client.connect()
            cached = await r.get(f"pcd:guest:device:{device_id}")
            if cached: return json.loads(cached)
        except: pass
        
        # Query Supabase for guest with matching oauth_id
        res = self.supabase_admin.table("players").select("*").eq("auth_provider", "guest").eq("oauth_id", device_id).execute()
        if res.data and len(res.data) > 0:
            guest = res.data[0]
            # Cache for 1 hour
            try:
                r = await redis_client.connect()
                await r.setex(f"pcd:guest:device:{device_id}", 3600, json.dumps(guest))
            except: pass
            return guest
        return None
